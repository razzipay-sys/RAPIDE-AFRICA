...
-- Root-cause cluster: profiles has no FK from riders/wallets/orders/
-- support_tickets, so PostgREST can't embed profiles in those queries
-- (dispatcher, admin/users, admin/drivers, support all silently return
-- empty/broken data). Plus: admin can't UPDATE profiles (KYC approval
-- was completely non-functional), escrows RLS is wide open, dispatcher
-- can't update orders, rider self-signup can't grant its own role, the
-- wallet ledger never actually moves money, and merchant API keys are
-- stored in plaintext.
--
-- NOTE ON SCHEMA DRIFT: supabase/migrations/20260519000000_platform_expansion.sql
-- defines support_tickets.description, but the live/generated schema
-- (src/integrations/supabase/types.ts) has support_tickets.message instead,
-- and the app code already reads/writes `message`. This migration does not
-- touch support_tickets' shape — only adds the missing FK — but the
-- repo's migration history should be reconciled with the live DB
-- separately (consider `supabase db diff` / re-pulling migrations).
-- ============================================================

-- ── 1. FK relationships so PostgREST can embed `profiles` ──────────────────
-- profiles.id is guaranteed to equal some auth.users.id 1:1 (handle_new_user
-- trigger inserts a profiles row for every new auth user), so adding a
-- second FK (alongside the existing auth.users FK) on these columns is a
-- safe, additive, non-breaking change — it does not alter any data.
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ── 2. profiles RLS: admin needs to be able to UPDATE (KYC approval), ──────
--     and support/dispatcher need to SELECT (for the embeds above to
--     actually return data under RLS, not just exist as a relationship).
...
-- ── 4. escrows RLS: close the "any authenticated user can update any ──────
--     escrow" hole. Nothing in the app writes to escrows client-side today
--     (app.escrow.tsx is read-only), so restricting writes to admin only
--     is a pure hardening change with no functional regression.
DROP POLICY IF EXISTS "System can manage escrows" ON public.escrows;
CREATE POLICY "admin_manage_escrows" ON public.escrows FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- ── 5. Rider self-onboarding: a brand-new user must be able to grant ──────
--     themselves the 'rider' role, but user_roles INSERT is admin-only by
--     RLS. This function hardcodes the role to 'rider' (never accepts a
...
-- ── 6. `documents` storage bucket — referenced by rider/documents.tsx and ──
--     app.verification.tsx but never provisioned; every upload 404s today.
...
CREATE POLICY "documents_public_read" ON storage.objects FOR SELECT TO public
...
-- ── 7. Wallet ledger: make it real ─────────────────────────────────────────
-- Add a status so a top-up can be recorded as "pending verification" and
-- only actually move money once an admin confirms the mobile-money receipt
-- (this is the flow app.wallet.tsx's UI already promises but nothing
-- backed — no code anywhere ever updated wallets.balance_xof).
ALTER TABLE public.wallet_transactions
  ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'completed'
    CHECK (status IN ('pending', 'completed', 'rejected'));

-- Any pre-existing topup rows were never actually credited (confirmed: no
-- code path updates balance_xof), so they are genuinely still pending,
-- not historically completed — backfill them honestly so ops can process
-- the real backlog through the new approval screen.
...
-- ── 8. complete_delivery: actually pay the rider ───────────────────────────
-- Extends the existing OTP-confirmation RPC to credit the rider's wallet
-- with their payout share once delivery is confirmed. Assumes cash-on-
-- delivery (rider already collected price_xof in cash from the customer)
-- so only the platform's commission is *not* credited to the rider — this
-- is the realistic default given there is no payment gateway integrated
-- yet and no payment-method selector in the booking flow. If/when a wallet-
-- prepaid checkout option is added, this function will need a matching
-- branch that credits the full price_xof (since the platform would have
-- already collected it from the customer at booking time).
...
BEGIN
  ...
  IF rows_affected = 1 THEN
    ...
    -- Credit rider payout (COD model: platform's cut, i.e. commission, is
    -- what the rider owes back — crediting price-commission is equivalent
    -- to letting the rider keep the cash minus the platform's share).
...
-- ── 9. Admin audit log — role grants (especially admin) need an audit trail ─
