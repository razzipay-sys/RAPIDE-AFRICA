...
function BookPage() {
  ...
  // Default pickup to the user's current location, like Uber/Bolt/Glovo —
  // falls back silently to the CITIES[0] default if permission is denied.
  useEffect(() => {
    locateMe();
  ...
  const pricing = useMemo(
    () => quote({ distanceKm: distance, type, insurance, weightKg: weight }),
    [distance, type, insurance, weight],
  );
  const etaMin = useMemo(() => Math.max(3, Math.round((distance / 20) * 60)), [distance]);
  ...
  const submit = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        dropoff_contact_name: dropName,
        dropoff_contact_phone: dropPhone,
        parcel_category: category,
        parcel_weight_kg: weight,
        parcel_notes: notes || null,
        delivery_type: type,
        insurance,
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] max-h-[850px] overflow-hidden pb-6">
      ...
      {/* Persistent fare/ETA bar — always visible, like Uber/Bolt's running fare preview */}
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-12 space-y-6 pt-4 relative">
        ...
        <div className={step === 1 ? "space-y-6" : "absolute opacity-0 pointer-events-none -z-10"}>
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">{t("book.step1.desc")}</p>
          </div>
          ...
          <div className="space-y-3 relative">
            ...
            <div className="relative z-10">
              <AddressSearch
                label={locatingUser ? t("book.locating") : t("book.pickup")}
                icon="A"
                value={pickup}
                onChange={setPickup}
          ...
          <div className="relative w-full h-[300px] rounded-3xl overflow-hidden border border-white/10 shadow-elegant bg-muted/20">
            {showMap ? (
              <Suspense fallback={<MapSkeleton height={300} />}>
                <LiveMap
                  pickup={pickup}
                  dropoff={dropoff}
                  height={300}
              ...
              <>
                <div className="absolute top-3 left-1/2 -translate-x-1/2 pointer-events-none z-10">
                  <div className="bg-background/80 backdrop-blur-md px-4 py-1.5 rounded-full shadow-lg border border-border/50 flex items-center gap-2">
                    <div
                      className={`h-2 w-2 rounded-full animate-pulse ${lastFocused === "pickup" ? "bg-primary" : "bg-blue-500"}`}
                    />
                    <span className="text-[10px] font-medium text-foreground uppercase tracking-wider">
                      {t("book.target")}{" "}
                      {lastFocused === "pickup"
                        ? t("book.target_pickup")
                        : t("book.target_dropoff")}
                    </span>
                  </div>
                </div>
              </>
            ...
            {showMap && (
              <button
                ...
                className="absolute bottom-3 left-3 z-10 h-9 w-9 rounded-xl bg-background/80 backdrop-blur-md border border-border/50 shadow-sm flex items-center justify-center hover:bg-background transition-colors disabled:opacity-50"
                title={t("book.use_current_location")}
              >
                {locatingUser ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              {CATEGORIES.map((c) => (
                ...
                >
                  ...
                  <span
                    className={`text-sm font-medium ${category === c.value ? "text-primary" : "text-foreground/80"}`}
                  >
                    {t(`book.cat.${c.value}` as Parameters<typeof t>[0])}
                  </span>
                </button>
              ))}
            </div>
        ...
        <div className={step === 3 ? "space-y-6" : "hidden"}>
          ...
          {/* Delivery Speeds — price shown per tier, like Uber/Bolt's ride selector */}
          <div className="space-y-3">
            <TypeCard
              active={type === "standard"}
              onClick={() => setType("standard")}
              icon={Package}
              title={t("book.std.title")}
              desc={t("book.std.desc")}
              price={fmtXOF(priceForType("standard"))}
            />
            <TypeCard
              active={type === "express"}
              onClick={() => setType("express")}
              icon={Zap}
              title={t("book.exp.title")}
              desc={t("book.exp.desc")}
              price={fmtXOF(priceForType("express"))}
              isPremium
            />
            <TypeCard
              ...
              icon={Clock}
              title={t("book.sched.title")}
              desc={t("book.sched.desc")}
              price={fmtXOF(priceForType("scheduled"))}
            />
          </div>
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`h-10 w-10 rounded-full flex items-center justify-center ${insurance ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
              >
            ...
            <div
              className={`h-6 w-6 rounded-full border-2 flex items-center justify-center transition-colors ${insurance ? "border-primary bg-primary" : "border-muted-foreground/30"}`}
            >
          ...
          <div className="bg-card border border-border rounded-3xl p-6 shadow-sm relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 right-0 h-1 flex justify-around opacity-20">
              {Array.from({ length: 20 }).map((_, i) => (
                <div key={i} className="w-2 h-2 bg-background rotate-45 -translate-y-1.5" />
              ))}
            </div>
    ...
    <div className="relative">
      <label
        className={`absolute left-4 transition-all duration-200 pointer-events-none z-10 ${
          active
            ? "text-[10px] top-1.5 text-primary font-semibold"
...
function TypeCard({
  ...
  return (
    ...
    >
      ...
      {price && (
        <span
          className={`text-sm font-display font-bold shrink-0 ${active ? "text-primary" : "text-foreground/70"}`}
        >
          {price}
        </span>
      )}
...
function ReceiptRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 text-sm">
      ...
      <span className={`font-semibold ${truncate ? "truncate max-w-[50%] text-right" : ""}`}>
        {value}
      </span>
    </div>
  );
