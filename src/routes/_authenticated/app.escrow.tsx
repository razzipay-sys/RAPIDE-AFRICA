  ...
  const { data: escrows, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("escrows")
        .select("*, orders(code, dropoff_address), seller:profiles!escrows_seller_id_fkey(full_name)")
        .eq("buyer_id", user!.id)
      ...
      <main className="flex-1 p-4 max-w-md mx-auto w-full space-y-4">
        <div className="bg-green-50 border border-green-100 rounded-xl p-4">
          ...
          <p className="text-xs text-green-800/80 mt-1">
            Your money is held safely until the item is delivered. If the item isn't delivered or doesn't match the description, you get a full refund.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {escrows.map(e => (
              <div key={e.id} className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide flex items-center gap-1 ${
                ...
                <div className="pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <p className="font-medium truncate">{(e.seller as any)?.full_name || 'Unknown'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground">Dropoff</p>
                    <p className="font-medium truncate">{(e.orders as any)?.dropoff_address || 'Pending'}</p>
                  </div>
                </div>
              </div>
            ))}
