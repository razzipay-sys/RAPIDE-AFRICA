...
function generateKey() {
  ...
  return "rpd_" + Array.from(arr).map((b) => chars[b % chars.length]).join("");
}
  ...
  const createKey = useMutation({
    mutationFn: async (name: string) => {
      ...
      const { error } = await supabase.from("merchant_api_keys").insert({
        ...
        key_hash: key, // In production: store bcrypt hash, return plaintext once
    ...
    onSuccess: (key) => {
      ...
      toast.success("API key created", {
        description: `${key.slice(0, 20)}… — copy it now, it won't be shown again`,
        duration: 8000,
      });
    },
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Integrate Rapide directly into your system</p>
        </div>
        <button
  ...
  -d '{"pickup_address":"...","dropoff_address":"..."}'`}
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2].map((i) => <div key={i} className="glass rounded-2xl h-20 animate-pulse" />)}
        </div>
      ) : keys?.length === 0 ? (
        <div className="text-center py-16 glass rounded-2xl">
          <Key className="h-10 w-10 text-muted-foreground/40 mx-auto mb-3" />
          <p className="font-semibold">No API keys yet</p>
          <p className="text-sm text-muted-foreground mt-1">Create your first key to start integrating</p>
        </div>
      ) : (
        <div className="space-y-3">
          {keys?.map((k) => (
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {!k.is_active && (
                      <span className="text-xs text-destructive glass rounded-full px-2 py-0.5">Revoked</span>
                    )}
                  </div>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">
                    ...
                    {k.last_used_at && ` · Last used ${new Date(k.last_used_at).toLocaleDateString("fr-FR")}`}
                  </p>
                </div>
